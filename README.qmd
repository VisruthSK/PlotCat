...
to SVG plots. `PlotCa`t obfuscates the target source in the rendered HTML,
...
options in document front matter. For project configuration, nesting them
under `live-html` avoids metadata-merging problems.
...
`coarse * svg-geometry-coarse + counts * svg-geometry-counts`. The
final score sums each category multiplied by its top-level weight.
