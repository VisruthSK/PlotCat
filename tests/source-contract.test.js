...
test('required stable classes are present', async () => {
  ...
  for (const name of ['plotcat__header','plotcat__body','plotcat__target','plotcat__student','plotcat__plot','plotcat__editor','plotcat__actions','plotcat__button','plotcat__status','plotcat__score','plotcat__compare','plotcat__controls','plotcat__wipe-handle','completion: true']) assert.match(lua, new RegExp(name));
  assert.doesNotMatch(lua, /runbutton/);
  ...
  assert.doesNotMatch(lua, /plotcat__feedback/);
});
